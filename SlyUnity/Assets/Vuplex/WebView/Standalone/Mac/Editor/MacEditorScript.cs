...
namespace Vuplex.WebView.Editor {
    ...
    /// <summary>
    /// Mac editor script that modifies the VuplexWebViewMac.bundle
    ...
    class MacEditorScript {
        ...
        static MacEditorScript() {
            ...
            var pluginPath = EditorUtils.FindDirectory("Assets/Vuplex/WebView/Standalone/Mac/Plugins/VuplexWebViewMac.bundle");
            ...
            foreach (var relativePath in binaryRelativePaths) {
                var path = Path.Combine(pluginPath, relativePath);
                _chmod("755 \"" + path + "\"");
            }
        }
